...
func (h *StorageImpl) UploadFromPath(ctx context.Context, path string, cb ProgressCallback) (*Storage, error) {
	...
	fileName, err := h.GenerateUniqueName(filepath.Base(path), contentType)
	if err != nil {
		return nil, err
	}
	storage := &Storage{
...
func (h *StorageImpl) UploadFromURL(ctx context.Context, url string, cb ProgressCallback) (*Storage, error) {
	...
	fileName, err = h.GenerateUniqueName(fileName, contentType)
	if err != nil {
		return nil, err
	}
	storage := &Storage{
...
func (h *StorageImpl) UploadFromBinary(ctx context.Context, data []byte, cb ProgressCallback) (*Storage, error) {
	...
	fileName, err := h.GenerateUniqueName("file", contentType)
	if err != nil {
		return nil, err
	}
	storage := &Storage{
...
func (h *StorageImpl) UploadFromHeader(ctx context.Context, header *multipart.FileHeader, cb ProgressCallback) (*Storage, error) {
	...
	fileName, err := h.GenerateUniqueName(header.Filename, contentType)
	if err != nil {
		return nil, err
	}
...
func (h *StorageImpl) GenerateUniqueName(original string, contentType string) (string, error) {
	ext := filepath.Ext(original)
	base := strings.TrimSuffix(original, ext)
	if ext == "" && contentType != "" {
		ext = helpers.GetExtensionFromContentType(contentType)
	}
