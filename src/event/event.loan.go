...
func (e *Event) LoanRelease(ctx context.Context, echoCtx echo.Context, tx *gorm.DB, data LoanBalanceEvent) (*model.LoanTransaction, error) {
	...
	for _, entry := range loanTransaction.LoanTransactionEntries {
		...
		if entry.Type == model.LoanTransactionPrevious {
			tx.Rollback()
