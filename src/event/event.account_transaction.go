...
func AccountTransactionProcess(
	...
	for currentDate := startDate; !currentDate.After(endDate); currentDate = currentDate.AddDate(0, 0, 1) {
		...
		if err := core.AccountTransactionDestroyer(context, service, tx, currentDate, userOrg.OrganizationID, *userOrg.BranchID); err != nil {
			return endTx(eris.Wrap(err, "failed to destroy previous account transactions"))
		}

		// ----- DAILY BOOKING -----
		booking, err := core.DailyBookingCollection(context, service, currentDate, userOrg.OrganizationID, *userOrg.BranchID)
		if err != nil {
		...
		if len(summary) > 0 {
			if err := core.AccountTransactionManager(service).CreateWithTx(context, tx, accountTransactionCollection); err != nil {
		...
		for _, s := range summary {
			...
			if err := core.AccountTransactionEntryManager(service).CreateWithTx(context, tx, entry); err != nil {
		...
		if len(summary) > 0 {
			if err := core.AccountTransactionManager(service).UpdateByIDWithTx(context, tx, accountTransactionCollection.ID, accountTransactionCollection); err != nil {
				return endTx(eris.Wrap(err, "failed to update daily booking transaction totals"))
			}
		}

		// ----- DAILY DISBURSEMENT -----
		disbursement, err := core.DailyDisbursementCollection(context, service, currentDate, userOrg.OrganizationID, *userOrg.BranchID)
		if err != nil {
		...
		if len(summary) > 0 {
			if err := core.AccountTransactionManager(service).CreateWithTx(context, tx, disbursementTransaction); err != nil {
		...
		if len(summary) > 0 {
			if err := core.AccountTransactionManager(service).UpdateByIDWithTx(context, tx, disbursementTransaction.ID, disbursementTransaction); err != nil {
				return endTx(eris.Wrap(err, "failed to update daily disbursement transaction totals"))
			}
		}

		// ----- DAILY JOURNAL -----
		journal, err := core.DailyJournalCollection(context, service, currentDate, userOrg.OrganizationID, *userOrg.BranchID)
		if err != nil {
		...
		if len(summary) > 0 {
			if err := core.AccountTransactionManager(service).CreateWithTx(context, tx, journalTransaction); err != nil {
		...
		if len(summary) > 0 {
			if err := core.AccountTransactionManager(service).UpdateByIDWithTx(context, tx, journalTransaction.ID, journalTransaction); err != nil {
	...
	for month := 1; month <= 12; month++ {
		...
		for _, entry := range accountTransactions {
			...
			resp := &types.AccountTransactionEntryResponse{
				...
				Account:        core.AccountManager(service).ToModel(entry.Account),
				Debit:          entry.Debit,
				Credit:         entry.Credit,
