...
func (m *Model) LoanTransactionEntriesCompute(
	context context.Context,
	userId uuid.UUID, orgId uuid.UUID, branchId uuid.UUID,
	cashOnHandAccountID uuid.UUID,
	...
	for _, entries := range entries {
		...
		if entries.Type == LoanTransactionAddOn {
			addOn = append(addOn, entries)
		}
		if entries.Type == LoanTransactionDeduction {
	...
	for count, value := range result {
		...
		value.LoanTransactionID = loanTransaction.ID
		value.CreatedAt = time.Now().UTC()
		value.UpdatedAt = time.Now().UTC()
	}

	automaticLoanDeductions, err := m.LoanTransactionEntryManager.Find(context, &LoanTransactionEntry{
		OrganizationID:    orgId,
		BranchID:          branchId,
...
func (m *Model) LoanComputation(ctx context.Context, ald AutomaticLoanDeduction, lt LoanTransaction) float64 {
	result := lt.Applied1
	// --- Min/Max check ---
