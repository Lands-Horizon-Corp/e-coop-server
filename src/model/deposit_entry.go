...
type (
	DepositEntry struct {
		...
		OrganizationID uuid.UUID     `gorm:"type:uuid;not null;index:idx_organization_branch_deposit_entry"`
		Organization   *Organization `gorm:"foreignKey:OrganizationID;constraint:OnDelete:CASCADE,OnUpdate:CASCADE;" json:"organization,omitempty"`
		BranchID       uuid.UUID     `gorm:"type:uuid;not null;index:idx_organization_branch_deposit_entry"`
...
func (m *Model) DepositEntry() {
	m.Migration = append(m.Migration, &DepositEntry{})
	m.DepositEntryManager = horizon_services.NewRepository(horizon_services.RepositoryParams[
		...
		Resource: func(data *DepositEntry) *DepositEntryResponse {
			if data == nil {
				return nil
			}
		...
		Created: func(data *DepositEntry) []string {
			return []string{
				"deposit_entry.create",
				fmt.Sprintf("deposit_entry.create.%s", data.ID),
				fmt.Sprintf("deposit_entry.create.branch.%s", data.BranchID),
				fmt.Sprintf("deposit_entry.create.organization.%s", data.OrganizationID),
			}
		},
		Updated: func(data *DepositEntry) []string {
			return []string{
				"deposit_entry.update",
				fmt.Sprintf("deposit_entry.update.%s", data.ID),
				fmt.Sprintf("deposit_entry.update.branch.%s", data.BranchID),
				fmt.Sprintf("deposit_entry.update.organization.%s", data.OrganizationID),
			}
		},
		Deleted: func(data *DepositEntry) []string {
			return []string{
				"deposit_entry.delete",
				fmt.Sprintf("deposit_entry.delete.%s", data.ID),
				fmt.Sprintf("deposit_entry.delete.branch.%s", data.BranchID),
				fmt.Sprintf("deposit_entry.delete.organization.%s", data.OrganizationID),
			}
		},
	})
}

func (m *Model) DepositEntryCurrentBranch(context context.Context, orgId uuid.UUID, branchId uuid.UUID) ([]*DepositEntry, error) {
	return m.DepositEntryManager.Find(context, &DepositEntry{
