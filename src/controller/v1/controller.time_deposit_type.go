...
func (c *Controller) TimeDepositTypeController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		timeDepositType, err := c.model.TimeDepositTypeManager.GetByIDRaw(context, *timeDepositTypeID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		RequestType:  model.TimeDepositTypeRequest{},
		ResponseType: model.TimeDepositTypeResponse{},
	}, func(ctx echo.Context) error {
		context := ctx.Request().Context()
		req, err := c.model.TimeDepositTypeManager.Validate(ctx)
		if err != nil {
		...
		return ctx.JSON(http.StatusCreated, c.model.TimeDepositTypeManager.ToModel(timeDepositType))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		timeDepositType, err := c.model.TimeDepositTypeManager.GetByID(context, *timeDepositTypeID)
		if err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		if err := c.model.TimeDepositTypeManager.UpdateFields(context, timeDepositType.ID, timeDepositType); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		return ctx.JSON(http.StatusOK, c.model.TimeDepositTypeManager.ToModel(timeDepositType))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		if err := c.model.TimeDepositTypeManager.DeleteByID(context, *timeDepositTypeID); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "delete-error",
	...
	req.RegisterRoute(handlers.Route{
		...
		for _, rawID := range reqBody.IDs {
			...
			if err := c.model.TimeDepositTypeManager.DeleteByIDWithTx(context, tx, timeDepositTypeID); err != nil {
				tx.Rollback()
				c.event.Footstep(context, ctx, event.FootstepEvent{
					Activity:    "bulk-delete-error",
