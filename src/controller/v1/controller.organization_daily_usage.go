...
func (c *Controller) OrganizationDailyUsage() {
	...
	req.RegisterRoute(handlers.Route{
		...
		ResponseType: model_core.OrganizationDailyUsageResponse{},
		...
		dailyUsage, err := c.model_core.GetOrganizationDailyUsageByOrganization(context, userOrg.OrganizationID)
		if err != nil {
		...
		return ctx.JSON(http.StatusOK, c.model_core.OrganizationDailyUsageManager.Filtered(context, ctx, dailyUsage))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		dailyUsage, err := c.model_core.OrganizationDailyUsageManager.GetByIDRaw(context, *dailyUsageId)
		if err != nil {
