...
func LoanTransactionController(service *horizon.HorizonService) {
	...
	req.RegisterWebRoute(horizon.Route{
		...
		userOrg, err := event.CurrentUserOrganization(context, service, ctx)
		if err != nil {
			return ctx.JSON(http.StatusUnauthorized, map[string]string{"error": "User authentication failed"})
		}
		if userOrg.UserType != types.UserOrganizationTypeOwner &&
			userOrg.UserType != types.UserOrganizationTypeEmployee {
		...
		if loanTransaction.OrganizationID != userOrg.OrganizationID ||
			loanTransaction.BranchID != *userOrg.BranchID {
			return ctx.JSON(http.StatusForbidden, map[string]string{"error": "Access denied"})
		}
		if loanTransaction.PrintedDate != nil {
			return ctx.JSON(http.StatusForbidden, map[string]string{
				"error": "Loan transaction already printed",
			})
		}
		...
		// 🧾 Increment OR if auto-generated
		if req.ORAutoGenerated {
			if err := event.IncrementOfficialReceipt(context, service, tx, req.CheckNumber, types.GeneralLedgerSourceLoan, userOrg); err != nil {
				return ctx.JSON(http.StatusForbidden, map[string]string{
					"error": "Loan transaction already printed: " + endTx(err).Error(),
				})
			}
		}
		...
		if err := core.LoanTransactionManager(service).UpdateByIDWithTx(context, tx, loanTransaction.ID, loanTransaction); err != nil {
			return ctx.JSON(
				...
				map[string]string{"error": endTx(err).Error()},
			)
		}

		if err := endTx(nil); err != nil {
			return ctx.JSON(http.StatusInternalServerError, map[string]string{"error": "Transaction failed: " + err.Error()})
		}

		event.Footstep(ctx, service, event.FootstepEvent{
			Activity:    "print-success",
			Description: "Successfully printed loan transaction",
			Module:      "LoanTransaction",
		})
		...
		if err != nil {
			return ctx.JSON(http.StatusInternalServerError, map[string]string{
				"error": "Failed to retrieve updated loan transaction: " + err.Error(),
			})
		}
