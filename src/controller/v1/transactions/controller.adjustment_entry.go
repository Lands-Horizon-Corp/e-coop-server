...
func AdjustmentEntryController(service *horizon.HorizonService) {
	...
	req.RegisterWebRoute(horizon.Route{
		...
		if req.ORAutoGenerated {
			if err := event.IncrementOfficialReceipt(context, service, tx, req.ReferenceNumber, types.GeneralLedgerSourceAdjustment, userOrg); err != nil {
		...
		if err := event.RecordTransaction(context, service, transactionRequest, types.GeneralLedgerSourceAdjustment, userOrg); err != nil {
			return ctx.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to record transaction: " + err.Error()})
		}
		if err := core.AdjustmentEntryManager(service).CreateWithTx(context, tx, adjustmentEntry); err != nil {
		...
		event.Footstep(ctx, service, event.FootstepEvent{
			...
			Description: "Created adjustment entry: " + adjustmentEntry.ReferenceNumber,
