...
func (c *Controller) UserRatingController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		Method:       "GET",
		ResponseType: model_core.UserRatingResponse{},
		...
		userRating, err := c.model_core.GetUserRater(context, *userId)
		if err != nil {
		...
		return ctx.JSON(http.StatusOK, c.model_core.UserRatingManager.Filtered(context, ctx, userRating))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		userRating, err := c.model_core.GetUserRatee(context, *userId)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		userRating, err := c.model_core.UserRatingManager.GetByID(context, *userRatingId)
		if err != nil {
			return ctx.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to retrieve user rating: " + err.Error()})
		}
		return ctx.JSON(http.StatusOK, c.model_core.UserRatingManager.ToModel(userRating))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		userRating, err := c.model_core.UserRatingCurrentBranch(context, userOrg.OrganizationID, *userOrg.BranchID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		Method:       "POST",
		ResponseType: model_core.UserRatingResponse{},
		RequestType:  model_core.UserRatingRequest{},
		Note:         "Creates a new user rating in the current user's branch.",
	}, func(ctx echo.Context) error {
		context := ctx.Request().Context()
		req, err := c.model_core.UserRatingManager.Validate(ctx)
		if err != nil {
		...
		userRating := &model_core.UserRating{
		...
		if err := c.model_core.UserRatingManager.Create(context, userRating); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "create-error",
	...
	req.RegisterRoute(handlers.Route{
		...
		if err := c.model_core.UserRatingManager.DeleteByID(context, *userRatingId); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "delete-error",
