...
func (c *Controller) TimesheetController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		Method:       "GET",
		ResponseType: model_core.TimesheetResponse{},
		...
		timesheet, _ := c.model_core.TimesheetManager.FindOne(context, &model_core.Timesheet{
			UserID:         user.UserID,
		...
		return ctx.JSON(http.StatusOK, c.model_core.TimesheetManager.ToModel(timesheet))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		Method:       "POST",
		RequestType:  model_core.TimesheetRequest{},
		ResponseType: model_core.TimesheetResponse{},
		...
		req, err := c.model_core.TimesheetManager.Validate(ctx)
		if err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "create-error",
		...
		if timesheet == nil || timesheet.TimeOut != nil {
			newTimesheet := &model_core.Timesheet{
				CreatedAt:      now,
			...
			if err := c.model_core.TimesheetManager.Create(context, newTimesheet); err != nil {
				c.event.Footstep(context, ctx, event.FootstepEvent{
					Activity:    "create-error",
			...
			return ctx.JSON(http.StatusOK, c.model_core.TimesheetManager.ToModel(newTimesheet))
		}
		...
		if err := c.model_core.TimesheetManager.UpdateFields(context, timesheet.ID, timesheet); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
	...
	req.RegisterRoute(handlers.Route{
		...
		timesheet, err := c.model_core.TimesheetManager.GetByIDRaw(context, *timesheetID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		timesheets, err := c.model_core.TimesheetCurrentBranch(context, user.OrganizationID, *user.BranchID)
		if err != nil {
			return ctx.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to retrieve timesheets: " + err.Error()})
		}
		return ctx.JSON(http.StatusOK, c.model_core.TimesheetManager.Filtered(context, ctx, timesheets))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		value, err := c.model_core.TimesheetCurrentBranch(context, user.OrganizationID, *user.BranchID)
		if err != nil {
		...
		return ctx.JSON(http.StatusOK, c.model_core.TimesheetManager.Pagination(context, ctx, value))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		timesheets, err := c.model_core.GetUserTimesheet(context, user.UserID, user.OrganizationID, *user.BranchID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		value, err := c.model_core.GetUserTimesheet(context, user.UserID, user.OrganizationID, *user.BranchID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		timesheets, err := c.model_core.GetUserTimesheet(context, *userID, user.OrganizationID, *user.BranchID)
		if err != nil {
			return ctx.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to retrieve user timesheets: " + err.Error()})
		}
		return ctx.JSON(http.StatusOK, c.model_core.TimesheetManager.Filtered(context, ctx, timesheets))
	...
	req.RegisterRoute(handlers.Route{
		...
		value, err := c.model_core.GetUserTimesheet(context, *userID, user.OrganizationID, *user.BranchID)
		if err != nil {
			return ctx.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to retrieve user timesheets for pagination: " + err.Error()})
		}
		return ctx.JSON(http.StatusOK, c.model_core.TimesheetManager.Pagination(context, ctx, value))
	...
	req.RegisterRoute(handlers.Route{
		...
		value, err := c.model_core.GetUserTimesheet(context, userOrganization.UserID, user.OrganizationID, *user.BranchID)
		if err != nil {
			return ctx.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to retrieve user timesheets for pagination: " + err.Error()})
		}
		return ctx.JSON(http.StatusOK, c.model_core.TimesheetManager.Pagination(context, ctx, value))
	...
	req.RegisterRoute(handlers.Route{
		...
		timesheets, err := c.model_core.TimeSheetActiveUsers(context, user.OrganizationID, *user.BranchID)
		if err != nil {
