...
func JournalVoucherController(service *horizon.HorizonService) {
	...
	req.RegisterWebRoute(horizon.Route{
		...
		var req types.JournalVoucherPrintRequest
		...
		if journalVoucher.OrganizationID != userOrg.OrganizationID ||
			journalVoucher.BranchID != *userOrg.BranchID {
			return ctx.JSON(http.StatusForbidden, map[string]string{"error": "Access denied"})
		}
		if journalVoucher.PrintedDate != nil {
			return ctx.JSON(http.StatusBadRequest, map[string]string{
				"error": "Journal voucher already printed",
			})
		}
		tx, endTx := service.Database.StartTransaction(context)
		if req.ORAutoGenerated {
			if err := event.IncrementOfficialReceipt(context, service, tx, req.CashVoucherNumber, types.GeneralLedgerSourceJournalVoucher, userOrg); err != nil {
		...
		if err := core.JournalVoucherManager(service).
			UpdateByIDWithTx(context, tx, journalVoucher.ID, journalVoucher); err != nil {
			return ctx.JSON(
