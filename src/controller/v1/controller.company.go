...
func (c *Controller) CompanyController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		ResponseType: model_core.CompanyResponse{},
		...
		companies, err := c.model_core.CompanyCurrentBranch(context, user.OrganizationID, *user.BranchID)
		if err != nil {
			return ctx.JSON(http.StatusNotFound, map[string]string{"error": "No companies found for the current branch"})
		}
		return ctx.JSON(http.StatusOK, c.model_core.CompanyManager.Filtered(context, ctx, companies))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		return ctx.JSON(http.StatusOK, c.model_core.CompanyManager.Pagination(context, ctx, companies))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		company, err := c.model_core.CompanyManager.GetByIDRaw(context, *companyID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		RequestType:  model_core.CompanyRequest{},
		ResponseType: model_core.CompanyResponse{},
	}, func(ctx echo.Context) error {
		context := ctx.Request().Context()
		req, err := c.model_core.CompanyManager.Validate(ctx)
		if err != nil {
		...
		if err := c.model_core.CompanyManager.Create(context, company); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "create-error",
		...
		return ctx.JSON(http.StatusCreated, c.model_core.CompanyManager.ToModel(company))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		company, err := c.model_core.CompanyManager.GetByID(context, *companyID)
		if err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		if err := c.model_core.CompanyManager.UpdateFields(context, company.ID, company); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		return ctx.JSON(http.StatusOK, c.model_core.CompanyManager.ToModel(company))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		if err := c.model_core.CompanyManager.DeleteByID(context, *companyID); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "delete-error",
	...
	req.RegisterRoute(handlers.Route{
		...
		for _, rawID := range reqBody.IDs {
			...
			names += company.Name + ","
			if err := c.model_core.CompanyManager.DeleteByIDWithTx(context, tx, companyID); err != nil {
				tx.Rollback()
				c.event.Footstep(context, ctx, event.FootstepEvent{
					Activity:    "bulk-delete-error",
		...
		c.event.Footstep(context, ctx, event.FootstepEvent{
			...
			Description: "Bulk deleted companies (/company/bulk-delete): " + names,
			Module:      "Company",
		})
		return ctx.NoContent(http.StatusNoContent)
	})
