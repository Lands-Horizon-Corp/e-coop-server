...
func (c *Controller) CompanyController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		ResponseType: model.CompanyResponse{},
		...
		companies, err := c.model.CompanyCurrentBranch(context, user.OrganizationID, *user.BranchID)
		if err != nil {
			return ctx.JSON(http.StatusNotFound, map[string]string{"error": "No companies found for the current branch"})
		}
		return ctx.JSON(http.StatusOK, c.model.CompanyManager.Filtered(context, ctx, companies))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		return ctx.JSON(http.StatusOK, c.model.CompanyManager.Pagination(context, ctx, companies))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		company, err := c.model.CompanyManager.GetByIDRaw(context, *companyID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		RequestType:  model.CompanyRequest{},
		ResponseType: model.CompanyResponse{},
	}, func(ctx echo.Context) error {
		context := ctx.Request().Context()
		req, err := c.model.CompanyManager.Validate(ctx)
		if err != nil {
		...
		if err := c.model.CompanyManager.Create(context, company); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "create-error",
		...
		return ctx.JSON(http.StatusCreated, c.model.CompanyManager.ToModel(company))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		company, err := c.model.CompanyManager.GetByID(context, *companyID)
		if err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		if err := c.model.CompanyManager.UpdateFields(context, company.ID, company); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		return ctx.JSON(http.StatusOK, c.model.CompanyManager.ToModel(company))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		if err := c.model.CompanyManager.DeleteByID(context, *companyID); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "delete-error",
	...
	req.RegisterRoute(handlers.Route{
		...
		for _, rawID := range reqBody.IDs {
			...
			names += company.Name + ","
			if err := c.model.CompanyManager.DeleteByIDWithTx(context, tx, companyID); err != nil {
				tx.Rollback()
				c.event.Footstep(context, ctx, event.FootstepEvent{
					Activity:    "bulk-delete-error",
