...
func (c *Controller) LoanStatusController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		Method:       "GET",
		ResponseType: modelcore.LoanStatusResponse{},
		...
		return ctx.JSON(http.StatusOK, c.modelcore.LoanStatusManager.Filtered(context, ctx, statuses))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		return ctx.JSON(http.StatusOK, c.modelcore.LoanStatusManager.Pagination(context, ctx, value))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		status, err := c.modelcore.LoanStatusManager.GetByIDRaw(context, *id)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		Method:       "POST",
		ResponseType: modelcore.LoanStatusResponse{},
		RequestType:  modelcore.LoanStatusRequest{},
		Note:         "Creates a new loan status record for the current user's organization and branch.",
	}, func(ctx echo.Context) error {
		context := ctx.Request().Context()
		req, err := c.modelcore.LoanStatusManager.Validate(ctx)
		if err != nil {
		...
		if err := c.modelcore.LoanStatusManager.Create(context, status); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "create-error",
		...
		return ctx.JSON(http.StatusCreated, c.modelcore.LoanStatusManager.ToModel(status))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		status, err := c.modelcore.LoanStatusManager.GetByID(context, *id)
		if err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		if err := c.modelcore.LoanStatusManager.UpdateFields(context, status.ID, status); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		return ctx.JSON(http.StatusOK, c.modelcore.LoanStatusManager.ToModel(status))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		if err := c.modelcore.LoanStatusManager.DeleteByID(context, *id); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "delete-error",
	...
	req.RegisterRoute(handlers.Route{
		...
		for _, rawID := range reqBody.IDs {
			...
			names += status.Name + ","
			if err := c.modelcore.LoanStatusManager.DeleteByIDWithTx(context, tx, id); err != nil {
				tx.Rollback()
				c.event.Footstep(context, ctx, event.FootstepEvent{
					Activity:    "bulk-delete-error",
