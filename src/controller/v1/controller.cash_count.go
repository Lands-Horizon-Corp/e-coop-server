...
func (c *Controller) CashCountController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		ResponseType: model_core.CashCountResponse{},
		...
		cashCount, err := c.model_core.CashCountCurrentBranch(context, user.OrganizationID, *user.BranchID)
		if err != nil {
			return ctx.JSON(http.StatusNotFound, map[string]string{"error": "No cash counts found for the current branch"})
		}
		return ctx.JSON(http.StatusOK, c.model_core.CashCountManager.Pagination(context, ctx, cashCount))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		cashCount, err := c.model_core.CashCountManager.Find(context, &model_core.CashCount{
	...
	req.RegisterRoute(handlers.Route{
		...
		return ctx.JSON(http.StatusOK, c.model_core.CashCountManager.Filtered(context, ctx, cashCounts))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		Method:       "POST",
		ResponseType: model_core.CashCountResponse{},
		RequestType:  model_core.CashCountRequest{},
		...
		context := ctx.Request().Context()
		var cashCountReq model_core.CashCountRequest
		...
		newCashCount := &model_core.CashCount{
		...
		if err := c.model_core.CashCountManager.Create(context, newCashCount); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "create-error",
		...
		return ctx.JSON(http.StatusCreated, c.model_core.CashCountManager.ToModel(newCashCount))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		type CashCountBatchRequest struct {
			CashCounts        []model_core.CashCountRequest `json:"cash_counts" validate:"required"`
		...
		if batchRequest.DeletedCashCounts != nil {
			for _, deletedID := range *batchRequest.DeletedCashCounts {
				if err := c.model_core.CashCountManager.DeleteByID(context, deletedID); err != nil {
					c.event.Footstep(context, ctx, event.FootstepEvent{
						Activity:    "update-error",
		...
		var updatedCashCounts []*model_core.CashCount
		for _, cashCountReq := range batchRequest.CashCounts {
			...
			if cashCountReq.ID != nil {
				...
				if err := c.model_core.CashCountManager.UpdateFields(context, *cashCountReq.ID, data); err != nil {
					c.event.Footstep(context, ctx, event.FootstepEvent{
						Activity:    "update-error",
				...
				updatedCashCount, err := c.model_core.CashCountManager.GetByID(context, *cashCountReq.ID)
				if err != nil {
					c.event.Footstep(context, ctx, event.FootstepEvent{
						Activity:    "update-error",
		...
		allCashCounts, err := c.model_core.CashCountManager.Find(context, &model_core.CashCount{
			TransactionBatchID: transactionBatch.ID,
			OrganizationID:     userOrg.OrganizationID,
			BranchID:           *userOrg.BranchID,
		})
		...
		var responseRequests []model_core.CashCountRequest
		for _, cashCount := range updatedCashCounts {
			responseRequests = append(responseRequests, model_core.CashCountRequest{
				ID:                 &cashCount.ID,
	...
	req.RegisterRoute(handlers.Route{
		...
		cashCount, err := c.model_core.CashCountManager.GetByID(context, *cashCountID)
		if err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "delete-error",
		...
		if err := c.model_core.CashCountManager.DeleteByID(context, *cashCountID); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "delete-error",
	...
	req.RegisterRoute(handlers.Route{
		...
		return ctx.JSON(http.StatusOK, c.model_core.CashCountManager.ToModel(cashCount))
	})
}
