...
func (c *Controller) FootstepController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		ResponseType: model_core.FootstepResponse{},
	}, func(ctx echo.Context) error {
		context := ctx.Request().Context()
		req, err := c.model_core.FootstepManager.Validate(ctx)
		if err != nil {
		...
		if err := c.model_core.FootstepManager.Create(context, footstep); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "create-error",
		...
		return ctx.JSON(http.StatusCreated, c.model_core.FootstepManager.ToModel(footstep))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		footstep, err := c.model_core.GetFootstepByUser(context, user.ID)
		if err != nil {
			return ctx.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to retrieve user's footsteps: " + err.Error()})
		}
		return ctx.JSON(http.StatusOK, c.model_core.FootstepManager.Pagination(context, ctx, footstep))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		footstep, err := c.model_core.GetFootstepByUserOrganization(context, *memberProfile.UserID, userOrg.OrganizationID, *userOrg.BranchID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		footstep, err := c.model_core.GetFootstepByBranch(context, userOrg.OrganizationID, *userOrg.BranchID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		targetUserOrg, err := c.model_core.UserOrganizationManager.GetByID(context, *userOrgId)
		if err != nil {
			return ctx.JSON(http.StatusNotFound, map[string]string{"error": "User organization not found"})
		}

		footstep, err := c.model_core.GetFootstepByUserOrganization(context, targetUserOrg.UserID, targetUserOrg.OrganizationID, *targetUserOrg.BranchID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		footstep, err := c.model_core.FootstepManager.GetByIDRaw(context, *footstepId)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		footstep, err := c.model_core.GetFootstepByUserOrganization(context, userOrg.UserID, userOrg.OrganizationID, *userOrg.BranchID)
		if err != nil {
