...
func (c *Controller) LoanTagController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		loanTags, err := c.model.LoanTagCurrentBranch(context, user.OrganizationID, *user.BranchID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		return ctx.JSON(http.StatusOK, c.model.LoanTagManager.Pagination(context, ctx, loanTags))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		loanTag, err := c.model.LoanTagManager.GetByIDRaw(context, *loanTagID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		RequestType:  model.LoanTagRequest{},
		ResponseType: model.LoanTagResponse{},
	}, func(ctx echo.Context) error {
		context := ctx.Request().Context()
		req, err := c.model.LoanTagManager.Validate(ctx)
		if err != nil {
		...
		if err := c.model.LoanTagManager.Create(context, loanTag); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "create-error",
		...
		return ctx.JSON(http.StatusCreated, c.model.LoanTagManager.ToModel(loanTag))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		loanTag, err := c.model.LoanTagManager.GetByID(context, *loanTagID)
		if err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		if err := c.model.LoanTagManager.UpdateFields(context, loanTag.ID, loanTag); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		return ctx.JSON(http.StatusOK, c.model.LoanTagManager.ToModel(loanTag))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		if err := c.model.LoanTagManager.DeleteByID(context, *loanTagID); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "delete-error",
	...
	req.RegisterRoute(handlers.Route{
		...
		for _, rawID := range reqBody.IDs {
			...
			names += loanTag.Name + ","
			if err := c.model.LoanTagManager.DeleteByIDWithTx(context, tx, loanTagID); err != nil {
				tx.Rollback()
				c.event.Footstep(context, ctx, event.FootstepEvent{
					Activity:    "bulk-delete-error",
