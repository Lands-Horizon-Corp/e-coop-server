...
func (c *Controller) LoanTagController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		ResponseType: model_core.LoanTagResponse{},
		...
		loanTags, err := c.model_core.LoanTagCurrentBranch(context, user.OrganizationID, *user.BranchID)
		if err != nil {
			return ctx.JSON(http.StatusNotFound, map[string]string{"error": "No loan tags found for the current branch"})
		}
		return ctx.JSON(http.StatusOK, c.model_core.LoanTagManager.Filtered(context, ctx, loanTags))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		loanTags, err := c.model_core.LoanTagManager.Find(context, &model_core.LoanTag{
	...
	req.RegisterRoute(handlers.Route{
		...
		return ctx.JSON(http.StatusOK, c.model_core.LoanTagManager.Pagination(context, ctx, loanTags))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		loanTag, err := c.model_core.LoanTagManager.GetByIDRaw(context, *loanTagID)
		if err != nil {
	...
	req.RegisterRoute(handlers.Route{
		...
		RequestType:  model_core.LoanTagRequest{},
		ResponseType: model_core.LoanTagResponse{},
	}, func(ctx echo.Context) error {
		context := ctx.Request().Context()
		req, err := c.model_core.LoanTagManager.Validate(ctx)
		if err != nil {
		...
		if err := c.model_core.LoanTagManager.Create(context, loanTag); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "create-error",
		...
		return ctx.JSON(http.StatusCreated, c.model_core.LoanTagManager.ToModel(loanTag))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		loanTag, err := c.model_core.LoanTagManager.GetByID(context, *loanTagID)
		if err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		if err := c.model_core.LoanTagManager.UpdateFields(context, loanTag.ID, loanTag); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		return ctx.JSON(http.StatusOK, c.model_core.LoanTagManager.ToModel(loanTag))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		if err := c.model_core.LoanTagManager.DeleteByID(context, *loanTagID); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "delete-error",
	...
	req.RegisterRoute(handlers.Route{
		...
		for _, rawID := range reqBody.IDs {
			...
			names += loanTag.Name + ","
			if err := c.model_core.LoanTagManager.DeleteByIDWithTx(context, tx, loanTagID); err != nil {
				tx.Rollback()
				c.event.Footstep(context, ctx, event.FootstepEvent{
					Activity:    "bulk-delete-error",
