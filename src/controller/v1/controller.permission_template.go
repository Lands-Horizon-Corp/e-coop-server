...
func (c *Controller) PermissionTemplateController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		Method:       "GET",
		ResponseType: model_core.PermissionTemplateResponse{},
		...
		permissionTemplates, err := c.model_core.GetPermissionTemplateByBranch(context, userOrg.OrganizationID, *userOrg.BranchID)
		if err != nil {
			return ctx.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to retrieve permission templates: " + err.Error()})
		}
		return ctx.JSON(http.StatusOK, c.model_core.PermissionTemplateManager.Filtered(context, ctx, permissionTemplates))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		return ctx.JSON(http.StatusOK, c.model_core.PermissionTemplateManager.Pagination(context, ctx, permissionTemplates))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		permissionTemplate, err := c.model_core.PermissionTemplateManager.GetByID(context, *permissionTemplateID)
		if err != nil {
			return ctx.JSON(http.StatusNotFound, map[string]string{"error": "Permission template not found: " + err.Error()})
		}

		return ctx.JSON(http.StatusOK, c.model_core.PermissionTemplateManager.ToModel(permissionTemplate))
	})
	...
	req.RegisterRoute(handlers.Route{
		Route:        "/api/v1/permission-template",
		Method:       "POST",
		RequestType:  model_core.PermissionTemplateRequest{},
		ResponseType: model_core.PermissionTemplateResponse{},
		Note:         "Creates a new permission template for the current user's branch.",
	}, func(ctx echo.Context) error {
		context := ctx.Request().Context()

		reqData, err := c.model_core.PermissionTemplateManager.Validate(ctx)
		if err != nil {
		...
		newTemplate := &model_core.PermissionTemplate{
		...
		if err := c.model_core.PermissionTemplateManager.Create(context, newTemplate); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "create-error",
		...
		return ctx.JSON(http.StatusOK, c.model_core.PermissionTemplateManager.ToModel(newTemplate))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		template, err := c.model_core.PermissionTemplateManager.GetByID(context, *permissionTemplateID)
		if err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		if err := c.model_core.PermissionTemplateManager.UpdateFields(context, template.ID, template); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		return ctx.JSON(http.StatusOK, c.model_core.PermissionTemplateManager.ToModel(template))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		if err := c.model_core.PermissionTemplateManager.DeleteByID(context, *permissionTemplateID); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "delete-error",
