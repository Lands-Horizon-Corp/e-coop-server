...
func (c *Controller) OnlineRemittanceController() {
	...
	req.RegisterRoute(handlers.Route{
		...
		Method:       "GET",
		ResponseType: model_core.OnlineRemittanceResponse{},
		...
		onlineRemittance, err := c.model_core.OnlineRemittanceManager.Find(context, &model_core.OnlineRemittance{
		...
		return ctx.JSON(http.StatusOK, c.model_core.OnlineRemittanceManager.Filtered(context, ctx, onlineRemittance))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		Method:       "POST",
		ResponseType: model_core.OnlineRemittanceResponse{},
		RequestType:  model_core.OnlineRemittanceRequest{},
		Note:         "Creates a new online remittance record for the current transaction batch.",
	}, func(ctx echo.Context) error {
		context := ctx.Request().Context()

		req, err := c.model_core.OnlineRemittanceManager.Validate(ctx)
		if err != nil {
		...
		onlineRemittance := &model_core.OnlineRemittance{
		...
		if err := c.model_core.OnlineRemittanceManager.Create(context, onlineRemittance); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "create-error",
		...
		allOnlineRemittances, err := c.model_core.OnlineRemittanceManager.Find(context, &model_core.OnlineRemittance{
			TransactionBatchID: &transactionBatch.ID,
			OrganizationID:     userOrg.OrganizationID,
			BranchID:           *userOrg.BranchID,
		})
		...
		return ctx.JSON(http.StatusOK, c.model_core.OnlineRemittanceManager.ToModel(onlineRemittance))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		existingOnlineRemittance, err := c.model_core.OnlineRemittanceManager.GetByID(context, *onlineRemittanceId)
		if err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		if err := c.model_core.OnlineRemittanceManager.UpdateFields(context, *onlineRemittanceId, existingOnlineRemittance); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		updatedRemittance, err := c.model_core.OnlineRemittanceManager.GetByID(context, *onlineRemittanceId)
		if err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "update-error",
		...
		return ctx.JSON(http.StatusOK, c.model_core.OnlineRemittanceManager.ToModel(updatedRemittance))
	})
	...
	req.RegisterRoute(handlers.Route{
		...
		if err := c.model_core.OnlineRemittanceManager.DeleteByID(context, *onlineRemittanceId); err != nil {
			c.event.Footstep(context, ctx, event.FootstepEvent{
				Activity:    "delete-error",
		...
		return ctx.JSON(http.StatusOK, c.model_core.OnlineRemittanceManager.ToModel(existingOnlineRemittance))
	})
}
