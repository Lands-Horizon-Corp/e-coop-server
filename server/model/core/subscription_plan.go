...
func (m *Core) subscriptionPlanSeed(ctx context.Context) error {
	...
	for _, currency := range currencies {
		...
		case "XDR": // Special Drawing Rights (IMF)
			subscriptions = []*SubscriptionPlan{
				newSubscriptionPlan("Enterprise Plan", "Enterprise level in XDR currency.", 300, 15, 25, "enterprise", &currency.ID),
				newSubscriptionPlan("Pro Plan", "Professional plan in XDR currency with AI features.", 150, 10, 20, "pro", &currency.ID),
				newSubscriptionPlan("Growth Plan", "Mid-sized organization plan in XDR currency.", 80, 8, 18, "growth", &currency.ID),
				newSubscriptionPlan("Starter Plan", "Starter plan in XDR currency for small orgs.", 40, 5, 15, "starter", &currency.ID),
				newSubscriptionPlan("Free Plan", "Basic free trial in XDR currency.", 0, 0, 0, "free", &currency.ID),
			}
		case "KWD": // Kuwait
			subscriptions = []*SubscriptionPlan{
